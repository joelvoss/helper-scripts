...
read -r -d '' usage <<-EOF || true
  ${CYAN}-p --project${RESET}  ${MAGENTA}[arg]${RESET}  GCP project id.
  ${CYAN}-z --zone${RESET}     ${MAGENTA}[arg]${RESET}  GCE zone.
...
read -r -d '' helptext <<-EOF || true
 Provision a google compute engine virtual machine.
EOF
...
# Create new user
printf "Creating a new user with username/password.\n"

printf "${DIM}>${RESET} Username: "
read USER
printf "${DIM}>${RESET} Password: "
read -s RAW_PASS
printf "\n"

USER_PASS=$(python -c 'import crypt; print crypt.crypt("${RAW_PASS}", "random_salt")')
# useradd -p ${USER_PASS} ${USER}

printf "User ${CYAN}${USER}${RESET} successfully created!\n"
